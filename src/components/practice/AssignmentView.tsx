    ...
    return (
        <>
            ...
            <div className="flex flex-col h-full">
                <div className="bg-white/80 dark:bg-black/70 backdrop-blur-sm py-4 z-10 border-b border-slate-200 dark:border-gray-800 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
                    <div className="max-w-7xl mx-auto flex justify-between items-center">
                        <h2 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white">SAT English Practice</h2>
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="text-2xl font-mono bg-slate-100 dark:bg-gray-900 text-slate-900 dark:text-white px-4 py-2 rounded-lg border border-slate-200 dark:border-gray-700">
                                {formatTime(timeLeft)}
                            </div>
                            <button onClick={() => setIsReviewPanelOpen(true)} className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-gray-800 transition-colors" aria-label="Review questions">
                                <GridIcon className="w-6 h-6 text-slate-500 dark:text-gray-300" />
                            </button>
                        </div>
                    </div>
                </div>

                <div className="flex-grow my-8 grid grid-cols-1 md:grid-cols-4 gap-8 min-h-0">
                    <div className={`relative transition-all duration-300 ${isHighlightMode ? 'md:col-span-3' : 'md:col-span-4'}`}>
                        ...
                        {currentQuestion && (
                            <div className="bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-gray-800 p-6 md:p-8 h-full" onMouseUp={handleMouseUp}>
                                <div className="flex justify-between items-center mb-4">
                                    <p className="font-semibold text-slate-900 dark:text-white">Question {currentQuestionIndex + 1} of {questions.length}</p>
                                    <HighlightToolbar 
                                ...
                                <div ref={questionContainerRef} className="select-text">
                                    ...
                                    {currentQuestion.options.map(opt => {
                                        ...
                                        return (
                                             <div key={opt.letter} className="flex items-center space-x-3">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`flex-shrink-0 p-1 rounded-full transition-colors ${
                                                            isStruckOut 
                                                                ...
                                                                : 'text-slate-400 dark:text-gray-500 hover:text-slate-800 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-gray-700'
                                                        }`}
                                                ...
                                                <label className={`block w-full p-4 rounded-lg border-2 transition-colors cursor-pointer ${userAnswers[currentQuestion.id] === opt.letter ? 'bg-purple-600/20 border-purple-500' : 'bg-white dark:bg-gray-800 border-slate-200 dark:border-gray-700 hover:border-purple-600'}`}>
                                                    <input
                                                        type="radio"
                ...
                <div className="flex justify-between items-center mt-auto pb-4">
                    <button onClick={() => setCurrentQuestionIndex(i => i - 1)} disabled={currentQuestionIndex === 0} className="inline-flex items-center space-x-2 bg-slate-200 dark:bg-gray-700 text-slate-800 dark:text-white font-bold py-3 px-6 rounded-lg hover:bg-slate-300 dark:hover:bg-gray-600 disabled:bg-slate-100 dark:disabled:bg-gray-800 disabled:text-slate-400 dark:disabled:text-gray-500 disabled:cursor-not-allowed transition-colors">
                        <ChevronLeftIcon className="w-5 h-5" />
                        <span>Previous</span>
                    </button>

                    {currentQuestionIndex < questions.length - 1 ? (
                        <button onClick={() => setCurrentQuestionIndex(i => i + 1)} className="inline-flex items-center space-x-2 bg-purple-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-purple-500 transition-colors">
                            <span>Next</span>
                            <ChevronRightIcon className="w-5 h-5" />
                        </button>
                    ) : (
                        <button onClick={handleSubmitClick} className="bg-green-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-green-700 transition-colors">
                            Submit Assignment
                        </button>
                    )}
                </div>
